...
class AuthController
{
    ...
    */
    public function register(ServerRequest $request)
    {
        ...
        $valid  = true;
        try {
            $userId = $this->auth->registerWithUniqueUsername($form['userEmail'], $form['userPassword'],
                $form['userName'], function ($selector, $token) use ($mailer, $form) {
                    ...
                    $mailer->sendEmail($form['userEmail'], Config::get('company_name'),
                        _('New Tracksz User Validation'), $message, ['url' => $url]);
                });
        }
    ...
    */
    public function verifyChange(ServerRequest $request, array $emailed)
    {
        ...
        $mailer->sendEmail($current_email, Config::get('company_name'),
            _('Your Tracksz Login Email Changed'), $message, []);
    ...
    */
    ...
    public function sendUsername(ServerRequest $request)
    {
        ...
        if (isset($username['username'])) {
            try {
                $mailer->sendEmail($form['email'], Config::get('company_name'),
                    _('Tracksz Username Request'), $message, ['username' => $username['username']]);
            } catch (Exception $e) {
                $valid = false;
            }
        }
        ...
        if (isset($email['email'])) {
            try {
                $this->auth->forgotPassword($email['email'], function ($selector, $token) use ($mailer, $email) {
                    ...
                    $mailer->sendEmail($email['email'], Config::get('company_name'),
                        _('Tracksz Reset Password Request'), $message, ['url' => $url]);
                }, 3600);
            } catch (\Delight\Auth\InvalidEmailException $e) {
                $valid = false;
